...
### Per-obligation proof verification

`workspace_verify_proof` accepts required `obligation_id`, optional configured
`repo`, and orchestrator-only optional `git_sha`. Read its live closed schema
...
An orchestrator exact-commit request requires the configured repository's
lowercase full object-width commit SHA. Refs, abbreviations, revision
expressions, all-zero identities, missing objects, and non-commit objects do not
fall back to the worktree. The server authenticates commit and tree and runs the
resolved declared test only in a private detached mode-0700 checkout. Omitting
`git_sha` authenticates HEAD/tree, dirty status, and the exact current-worktree
source snapshot, copies those bytes to a disposable private candidate, and
never executes in the configured worktree. Dirty snapshot proof is ephemeral
proof of those bytes, not of HEAD, a commit, or deployment.

The result carries complete `git_commit` or `worktree_snapshot` candidate
identity. Source/candidate movement, mismatch, escape, and cleanup failures are
typed refusals; a missing exact object can be reason-coded `not_executable`.
Normal proof outcomes are `satisfied`, `unsatisfied`, or reason-coded
`not_executable`. Private candidates are cleaned after success and failure.
