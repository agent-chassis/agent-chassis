...
- `packages/wiki-core/data/tool-discovery/controlled-contract-tools.json` —
  controlled-contract discovery, assessment, authoring, and proof routes.
...
1. `mcp-tools.json`
2. `controlled-contract-tools.json`
...
There is no separate “manifest-source digest.” `digestToolDiscoveryDescriptor`
hashes the assembled descriptor owned by the ordered fragment registry, and
`digestToolDiscoveryEntry` hashes individual rows for the conformance-debt
baseline. The manifest itself owns ordering, counts, the debt owner, target WK,
retirement evidence, and exact baseline entry digests; it does not claim an
independent authority digest.
