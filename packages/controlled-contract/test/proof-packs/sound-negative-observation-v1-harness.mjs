...
const EXACT_BINDING_SOURCES = Object.freeze({
  ...
  "observation-evidence": {
    kind: "artifact_file", relative_path: "observation-evidence.json"
  },
  "observation-projection": {
    kind: "artifact_file", relative_path: "observation-projection.json"
  }
});
...
async function createSoundNegativeObservationSubject({
  fixtureOptions = {},
  source = null,
  ...
  const captured = source ?? buildSoundNegativeObservationSources({
    conclusion: "absent", sourceCount: 2, observationCount: 2,
    domain: "sound-negative-exact", ...fixtureOptions
  });
  ...
  const profile = mutateProfile(buildSoundNegativeObservationProfile());
  const declaration = mutateDeclaration(
    buildSoundNegativeObservationDeclaration(profile), profile
  );
  ...
  const evaluationInput = mutateEvaluationInput(
    buildSoundNegativeObservationEvaluationInput(projectedContract), projectedContract
  );
  const files = sourceFiles ?? {
    "observation-evidence.json": captured.evidenceBytes,
    "observation-capture-proof.json": captured.captureProofBytes,
    "observation-projection.json": effectiveProjectionBytes
  };
  const root = await mkdtemp(path.join(os.tmpdir(), "cc-sound-negative-"));
