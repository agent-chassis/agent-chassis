...
const BOUNDED_STATE_STABILITY_V1_PROFILE = JSON.parse(await readFile(new URL(
  "../certification/profiles/proof.lifecycle.bounded-state-stability/2.0.0/profile.json",
  import.meta.url
), "utf8"));
const INPUT_TEMPLATE = JSON.parse(await readFile(new URL(
  "../certification/profiles/proof.lifecycle.bounded-state-stability/2.0.0/evaluation-input.template.json",
  import.meta.url
), "utf8"));
...
function identityFor(role, referenceId, domain) {
  if (role === "verification") return {
    ...
    path: "tests/bounded-state-stability.test.mjs", symbol: "boundedStateStability"
  };
  if (role.endsWith("_condition")) return {
    kind: "profile_term", term: `bounded-state-stability:${referenceId.slice(4)}`
  };
  return { kind: "durable_id", domain: `bounded-state-stability:${domain}`,
...
function buildBoundedStateStabilityFixture({
  profile: suppliedProfile = BOUNDED_STATE_STABILITY_V1_PROFILE,
  ...
  const defaultObservations = Array.from({ length: observationCount }, (_, index) =>
    `ref-observation-${index + 1}`);
  for (const binding of input.reference_bindings) {
    const defaultIds = binding.role === "observations"
      ? defaultObservations : binding.reference_ids;
    binding.reference_ids = [...(roleIdOverrides[binding.role] ?? defaultIds)];
  }
  const roleIds = bindingMap(input);
  for (const binding of input.number_bindings) {
    const defaultValue = binding.role === "observation_count"
      ? roleIds.get("observations").length : binding.value;
    binding.value = numberValueOverrides[binding.role] ?? defaultValue;
  }
...
export { BOUNDED_STATE_STABILITY_V1_PROFILE, buildBoundedStateStabilityFixture,
