...
function identityFor(role, referenceId, domain) {
  if (role === "verification") return { kind: "code_symbol", repository: `fixture-${domain}`,
    path: "tests/retry-convergence.test.mjs", symbol: "retryConvergence" };
  if (role.endsWith("_condition")) return { kind: "profile_term",
    term: `retry-convergence:${referenceId.slice(4)}` };
  return { kind: "durable_id", domain: `retry-convergence:${domain}`,
...
function buildRetryConvergenceFixture({ profile: suppliedProfile = RETRY_CONVERGENCE_V1_PROFILE,
  domain = "multipart-upload", role_id_overrides: roleIdOverrides = {},
...
export { RETRY_CONVERGENCE_V1_PROFILE, buildRetryConvergenceFixture,
