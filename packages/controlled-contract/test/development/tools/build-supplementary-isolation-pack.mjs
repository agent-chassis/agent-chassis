...
import { buildSupplementaryIsolationSources } from "../../proof-packs/supplementary-isolation-v1-fixture.mjs";
import {
  buildSupplementaryIsolationDeclaration,
  buildSupplementaryIsolationEvaluationInput,
  buildSupplementaryIsolationProfile
} from "../../proof-packs/supplementary-isolation-v1-profile.mjs";
import { EXCLUSIONS, GUARANTEE } from "../../proof-packs/supplementary-isolation-v1-constants.mjs";
import {
  CONTROL_IDS,
  runExactBindingCertificationControls
} from "../../proof-packs/supplementary-isolation-v1-exact-corpus.mjs";
import { runProofPackAdequacyControls } from "../../proof-packs/supplementary-isolation-v1-adequacy.mjs";

const packageRoot = path.resolve(fileURLToPath(new URL("../../../", import.meta.url)));
const repositoryRoot = path.resolve(packageRoot, "../..");
const certificationDirectory = path.join(packageRoot, "test/certification/profiles",
  "proof.failure.supplementary-isolation", "1.0.0");
const relative = (absolutePath) => path.relative(repositoryRoot, absolutePath).replaceAll("\\", "/");

async function digest(absolutePath) { return sha256(await readFile(absolutePath)); }
...
function staticDependencies(source) {
  ...
  const resourcePattern = /new\s+URL\(\s*["']([^"']+)["']\s*,\s*import\.meta\.url\s*\)/gu;
...
async function dependencyClosure(entryPath) {
  ...
  return Promise.all([...seen].sort().map(async (absolutePath) => ({
    path: relative(absolutePath), sha256: await digest(absolutePath)
  })));
}
...
const templateCapture = buildSupplementaryIsolationSources({ branch: "omitted" });
const template = buildSupplementaryIsolationEvaluationInput(
  JSON.parse(templateCapture.projectionBytes.toString("utf8"))
);
const adequacyModule = path.join(packageRoot, "test/proof-packs/supplementary-isolation-v1-adequacy.mjs");
const adequacyRun = await runProofPackAdequacyControls({ profile, profile_digest: profileDigest });
const controlIds = (category) => adequacyRun.controls.filter((control) =>
  control.category === category).map(({ control_id: id }) => id).sort();
const adequacy = {
  ...
  profile_digest: profileDigest, guarantee: GUARANTEE,
...
if (exactResult.failed_control_ids.length > 0) throw new Error(
  `exact controls failed: ${exactResult.failed_control_ids.join(", ")}`
);
const exactModule = path.join(packageRoot,
  "test/proof-packs/supplementary-isolation-v1-exact-corpus.mjs");
const exactCertification = {
  ...
  corpus: {
    corpus_id: "proof-failure-supplementary-isolation-exact-binding",
    corpus_version: "1.0.0",
    corpus_digest: sha256(canonicalJsonBytes({
      corpus_id: "proof-failure-supplementary-isolation-exact-binding",
      corpus_version: "1.0.0", controls: CONTROL_IDS
    })),
  ...
  result: { status: "passed", passed_control_ids: exactResult.passed_control_ids,
    failed_control_ids: [] }
};

await mkdir(certificationDirectory, { recursive: true });
await Promise.all([
  writeFile(path.join(certificationDirectory, "profile.json"), `${JSON.stringify(profile, null, 2)}\n`),
...
process.stdout.write(`${JSON.stringify({
  ...
  positive_count: controlIds("positive").length,
  mutant_count: controlIds("mutant").length,
  profile_weakening_and_splice_count: controlIds("profile_rejection").length,
