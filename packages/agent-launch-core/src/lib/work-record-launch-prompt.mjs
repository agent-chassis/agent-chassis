...
function structuredRoleResultExample({ role, subject }) {
  if (role === "worker") {
    return {
      ...
      summary: "Completed the assigned implementation work and validation.",
      findings: [],
...
export function buildLaunchPrompt({
  ...
  const lines = [
    ...
    "You read your assigned read_scope, repo_paths, and write_scope through the launcher-granted source-read access for this session: normally the launcher-owned structured filesystem-MCP read tool surface, and when no structured reader is configured for the session, native read-only inspection of that assigned scope from the read-only repo mount is the authorized fallthrough. Do not report a blocker merely because a structured filesystem-MCP reader is absent when this launcher-granted read access to your assigned scope is available.",
    ...
    "You may make in-place edits to the files in your assigned write_scope; the exact-file write_scope/bwrap bind is the gate for those edits. For Codex, use the directly granted apply_patch tool for those in-place edits instead of Edit/Write, shell, Bash, or raw exec_command.",
    ...
    "This launcher-granted read/edit access takes precedence over the AGENTS.md \"structured tools first / shell denied\" default: it is the session's authorized mechanism, is not raw shell, and does not require a structured reader to be present.",
    "",
