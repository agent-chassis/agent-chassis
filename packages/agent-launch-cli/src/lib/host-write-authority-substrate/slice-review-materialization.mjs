...
function worktreeRegistrationSnapshot(runGit, mainRepo) {
  ...
  return { raw: String(result.stdout ?? ""), records: parseWorktreeRegistrations(result.stdout) };
}
...
function resolveOrigHeadSnapshot(runGit, gitContext) {
  const result = runGit({ ...gitContext, args: ["rev-parse", "--verify", "--quiet", "ORIG_HEAD"] });
  if (result?.ok === true) return `present:${assertOid(String(result.stdout ?? "").trim(), "ORIG_HEAD")}`;
  if (result?.status === 1 || result?.status === 128) return "absent";
  fail(
    SLICE_REVIEW_MATERIALIZATION_DIAGNOSTIC_CODES.OBJECT_MISMATCH,
    "could not inspect ORIG_HEAD",
    { status: result?.status ?? null, stderr: result?.stderr ?? result?.error ?? null }
  );
}
...
function resolveTrustedState({
  ...
  const objectDirectory = gitOutput(runGit, gitContext, ["rev-parse", "--path-format=absolute", "--git-path", "objects"], {
    code: SLICE_REVIEW_MATERIALIZATION_DIAGNOSTIC_CODES.OBJECT_MISMATCH,
  ...
  return Object.freeze({
    ...
    gitDir: actualGitDir,
    objectDirectory,
    registrationRaw: registration.raw,
    refsSnapshot: gitOutput(runGit, { repo: mainRepo }, ["for-each-ref", "--format=%(refname) %(objectname)"], {
      code: SLICE_REVIEW_MATERIALIZATION_DIAGNOSTIC_CODES.OBJECT_MISMATCH,
      message: "could not snapshot repository refs"
    }),
    origHeadSnapshot: resolveOrigHeadSnapshot(runGit, gitContext)
  });
}
