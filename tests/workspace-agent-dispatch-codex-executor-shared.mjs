...
} from "../packages/agent-launch-cli/src/lib/workspace-agent-dispatch-codex-executor.mjs";
import {
...
const EXECUTOR_MODULE_PATH = path.resolve(
  THIS_DIR,
  "..",
  "packages/agent-launch-cli/src/lib/workspace-agent-dispatch-codex-executor.mjs"
);

const FINAL_RESULT_MODULE_PATH = path.resolve(
  THIS_DIR,
  "..",
  "packages/agent-launch-cli/src/lib/workspace-agent-codex-final-result.mjs"
);
...
const FAKE_ACCEPTANCE_CRITERIA = Object.freeze(["fake acceptance criterion"]);
const FAKE_ACCEPTANCE_VALIDATION = Object.freeze(["fake validation command"]);

async function fakeLoadWorkRecord({ id }) {
  return {
    valid: true,
    record: {
      id,
...
function createCodexTestBackend(options = {}) {
...
function fakeSourceToolSurface(overrides = {}) {
  const request = {
...
export {
  ...
  FINAL_RESULT_MODULE_PATH,
  FakeChild,
  fakeBwrapPlan,
  fakePlanForRole,
  FAKE_ACCEPTANCE_CRITERIA,
  FAKE_ACCEPTANCE_VALIDATION,
  fakeLoadWorkRecord,
  buildHarness,
  createCodexTestBackend,
  SAMPLE_INPUT,
  fakeSourceToolSurface
};
