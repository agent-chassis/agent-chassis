...
test("summary-budget shape never substitutes for authenticated launcher lineage", () => {
  const p = projection();
  p.summary_budget = { soft_limit_chars: 16_384, actual_chars: 1, exceeded: false };
  let verifierCalls = 0;
  const result = createReviewerLineageCapability({
    verifyLauncherProjection: () => {
      verifierCalls += 1;
      return true;
    }
  }).validateReviewerActionLineage(p, expected);
