...
test("Claude server/discover close is identified without treating arbitrary pre-initialize close as a probe", () => {
  const inner = createFakeInnerTransport();
  const { transport, events } = attach(inner);
  transport.onclose = () => {};
  inner.deliver({
  ...
  assert.notEqual(LAUNCHER_READINESS_SCHEMA_VERSIONS.CLIENT_DISCOVERY_PROBE_CLOSED,
    LAUNCHER_READINESS_SCHEMA_VERSIONS.CLIENT_CLOSED);

  const ordinaryInner = createFakeInnerTransport();
  const ordinary = attach(ordinaryInner);
  ordinary.transport.onclose = () => {};
  ordinaryInner.onclose();
...
test("the discovery-probe mark is the exact observed shape and nothing wider", async (t) => {

  const DISCOVER = Object.freeze({
    jsonrpc: "2.0", id: "server-discover-probe-1", method: "server/discover", params: {}
  });

  await t.test("an errored close is not a graceful probe close", () => {
    const inner = createFakeInnerTransport();
    const { transport, events } = attach(inner);
    transport.onclose = () => {};
    transport.onerror = () => {};
    inner.deliver(DISCOVER);
    inner.onerror(new Error("connection reset by peer"));
    inner.onclose();
    assert.deepEqual(events, [{
      schema_version: LAUNCHER_READINESS_SCHEMA_VERSIONS.CLIENT_CLOSED,
      closed: true
  ...
  await t.test("a probe that then initializes is a real session, not a probe", async () => {
    const inner = createFakeInnerTransport();
    const { transport, events } = attach(inner);
    transport.onclose = () => {};
    inner.deliver(DISCOVER);
    inner.deliver(INITIALIZE);
    inner.onclose();
    assert.equal(events.some((event) => event.discovery_probe === true), false,
      "an initialize on this connection retires any probe interpretation");
  });

  await t.test("server/discover AFTER initialize is not a probe", () => {
    const inner = createFakeInnerTransport();
    const { transport, events } = attach(inner);
    transport.onclose = () => {};
    inner.deliver(INITIALIZE);
  ...
  await t.test("a restart-failed lifecycle is never re-read as a probe", () => {
    const inner = createFakeInnerTransport();
    const { transport, events } = attach(inner);
    transport.onclose = () => {};
    inner.deliver(DISCOVER);
    ...
    assert.equal(
      events.some((event) =>
        event.schema_version === LAUNCHER_READINESS_SCHEMA_VERSIONS.CLIENT_RESTARTED),
      true, "the duplicate initialize is still typed restart evidence");
    assert.equal(events.some((event) => event.discovery_probe === true), false);
  });

  await t.test("a non-request server/discover shape is not a probe", () => {
    const inner = createFakeInnerTransport();
    const { transport, events } = attach(inner);
    transport.onclose = () => {};
