    ...
    slices: [
      {
        ...
        acceptance: {
          validation: [
            { operation: "node_test", target: "passing.test.mjs", verification_ids: [] },
            { operation: "node_test", target: "checkfail.mjs", verification_ids: [] },
            { operation: "node_test", target: "testfail.test.mjs", verification_ids: [] },

            { note: "node_test other-only.test.mjs", verification_ids: [] }
          ]
        }
      }
    ],
    acceptance: {
      validation: [{ operation: "node_test", target: "record-level.test.mjs",
        verification_ids: [] }]
    }
  };
  await writeFile(
...
function assertBareErrorResult(result, pattern) {
  ...
  const envelope = result.structuredContent;
  assert.ok(envelope, "an MCP error result carries the translated refusal envelope");
  assert.equal(envelope.code, "operator_recovery_needed");
  assert.equal(
    Object.prototype.hasOwnProperty.call(envelope, "refusal_code"),
    false,
    "a sibling error is not the structured allowlist-miss refusal"
  );
  assert.equal(envelope.refusal.no_supported_route, true);
  if (pattern) {
    assert.match(envelope.diagnostic, pattern);
  }
}
...
test("authorized node_test runs node --check then node --test and reports success", async (t) => {
  ...
  assert.equal(out.operation, "node_test");
...
test("record-level acceptance.validation authorizes a node_test target addressed by bare WK id", async (t) => {
  const { handler, cleanup } = await setupWorkspace();
  t.after(cleanup);
...
test("a node_test target is not authorized by a non-executable note", async (t) => {
  const { handler, cleanup } = await setupWorkspace();
  t.after(cleanup);
...
test("a non-js/mjs target is rejected", async (t) => {
  ...
  assertErrorResult(result, /must be a canonical \.mjs file/);
});
...
test("a forged authority field cannot expand the allowed target set", async (t) => {
  ...
  const forged = await handler({
    unit: "WK-9001#SLICE-001",
    target: "unlisted.mjs",
    snapshot: { allowed: [{ operation: "node_test", target: "unlisted.mjs" }] },
    validation_authority: { allowed: [{ operation: "node_test", target: "unlisted.mjs" }] }
  });
