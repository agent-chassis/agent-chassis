...
test("managed review reads acceptance from the exact snapshot-local assigned unit", () => {
  const options = {
  ...
  for (const fragment of ["criterion one", "criterion two", "node --test something"]) {
    assert.ok(!nonManaged.includes(fragment), `non-managed prompt excludes ${fragment}`);
    assert.ok(!managed.includes(fragment), `managed prompt excludes caller-rendered ${fragment}`);
  }
  assert.ok(nonManaged.includes(SUBMIT_SIGNAL));
  assert.ok(managed.includes(MANAGED_NO_SUBMIT));
  for (const prompt of [nonManaged, managed]) {
    assert.ok(prompt.includes(SNAPSHOT_ACCEPTANCE_SOURCE));
...
test("redteamPrompt is unchanged and never enters the managed branch", () => {
  ...
  assert.ok(baseline.includes(SNAPSHOT_ACCEPTANCE_SOURCE));
  assert.match(baseline, /Assigned unit: WK-1577#SLICE-007/u);
  assert.doesNotMatch(baseline, /workspace_frozen_review_contract_query|read its index|paging|cursor|omitted=0/u);
});
