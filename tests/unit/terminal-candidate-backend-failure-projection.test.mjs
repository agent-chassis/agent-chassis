...
import { createBackendTerminalCandidateCoordination } from "../../packages/agent-launch-cli/src/lib/workspace-agent-dispatch-backend-terminal-candidate-coordination.mjs";
import {
...
const BACKEND_SOURCE_URL = new URL(
  "../../packages/agent-launch-cli/src/lib/workspace-agent-dispatch-backend-terminal-candidate-coordination.mjs",
  import.meta.url
);
const RUNTIME_SOURCE_URL = new URL(
  "../../packages/wiki-mcp/src/lib/dispatch-terminal-candidate-coordinator.mjs",
  import.meta.url
);
...
async function refuseThrown(thrown, module = { createBackendTerminalCandidateCoordination }) {
  let recoveryCalls = 0;
  const ctx = backendContext(async () => {
    recoveryCalls += 1;
    throw thrown;
  });
  const backend = module.createBackendTerminalCandidateCoordination(ctx);
...
test("forged recovery authority is refused before any Git observation", async () => {
  const gitCalls = [];
  ...
  await assert.rejects(
    coordinator.recoverTerminalCandidateUnderAuthority({
      wkId: "WK-1783",
      authorityContext: Object.freeze({ forged: true })
    }),
    (error) => error?.code === UNTRUSTED_RUNNER_CODE
  );
...
test("WK-1783#SLICE-002 redacts a production-default Git failure for a missing secret-bearing path", async () => {
  ...
  const gitFailure = await defaultTerminalCandidateRunGit({
    repo: missingPath,
    args: ["rev-parse", "HEAD"]
  });
...
test("WK-1783 loaded runner-identity mutations cannot mint authentication", async (t) => {
  ...
  killed.push(await assertSemanticMutationKilled(
    ...
    registrationMutant,
    async (module) => {
      const error = await rejectedCoordinatorCarrier(typedSource, {
        module,
        mainRepo: root,
        worktreeRoot: root
      });
