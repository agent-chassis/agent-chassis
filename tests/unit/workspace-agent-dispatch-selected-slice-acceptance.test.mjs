...
function recordWithSliceAcceptance(sliceAcceptance) {
  return {
    ...
    acceptance: {
      ...
      validation: ["node --test tests/unit/workspace-agent-dispatch-selected-slice-acceptance.test.mjs"]
    },
    sections: {
  ...
  const dispatchBackend = new Proxy(
    {},
    {
      get(_target, prop) {
        ...
        if (prop === "resolveBackendRoutingDecision") return undefined;
        return (...backendArgs) => {
...
const VALID_VALIDATION = [
  "node --test tests/unit/workspace-agent-dispatch-selected-slice-acceptance.test.mjs"
];
...
for (const variant of SELECTED_SLICE_ACCEPTANCE_VARIANTS) {
  test(
    `registered dispatch refuses ${variant.name} with its exact readiness code before backend invocation`,
    async (t) => {
