...
test("selected_slice preserves complete current validation declarations", async () => {
  await withTempDir(async (tempDir) => {
    await bootstrapRepo({ dir: tempDir, repo: "agent-chassis/projection-structured-validation-test" });
    const fixture = buildTrackerFixture("WK-9918", 2);
    fixture.sections.why_it_matters = "Current selected-unit validation reads must remain lossless.";
    fixture.sections.scope = { items: ["selected-unit projection"], out_of_scope: [] };
    fixture.sections.tasks = [];
    fixture.sections.references = [];
    const selected = fixture.slices.find((slice) => slice.id === "active-slice");
    selected.acceptance.validation = [
      "Inspect the selected test evidence manually.",
      {
        operation: "node_test",
    ...
    assert.deepEqual(
      validateWorkRecord(fixture, {
        sourcePath: `wiki/work-records/${fixture.id}.json`
      }),
      [],
      "the current validation public-read fixture must remain canonical"
    );
    await installFixture(tempDir, fixture);
    ...
    const result = await getWikiRecord({
      dir: tempDir,
      id: fixture.id,
      selected_slice: selected.id
    });

    assert.equal(result.valid, true);
    assert.equal(result.selected_slice_found, true);
    const expectedValidation = [
      selected.acceptance.validation[0],
      {
        ...selected.acceptance.validation[1],
        verification_ids: [...selected.acceptance.validation[1].verification_ids].sort()
      }
    ];
    assert.deepEqual(result.selected_slice.acceptance.validation, expectedValidation);
    assert.equal(Object.hasOwn(result.selected_slice, "validation"), false);
    assert.notEqual(
      result.selected_slice.acceptance.validation[1],
      selected.acceptance.validation[1]
    );
    assert.deepEqual(Object.keys(result.selected_slice.acceptance.validation[1]), [
      "operation",
      "target",
      "verification_ids"
    ]);
...
test("work-record summary selected slice: includes selected notes and shared byte count", async () => {
  await withTempDir(async (tempDir) => {
    ...
    assert.deepEqual(stringNoteResult.summary.selected_unit_summary.validation,
...
test("work-record summary preserves current structured validation facts", async () => {
  await withTempDir(async (tempDir) => {
    await bootstrapRepo({ dir: tempDir, repo: "agent-chassis/projection-summary-validation-test" });
    const fixture = buildTrackerFixture("WK-9920", 1);
    const selected = fixture.slices.find((slice) => slice.id === "active-slice");
    selected.acceptance.validation = [{
      operation: "node_test",
      target: "tests/active.test.mjs",
      verification_ids: ["claim-active"]
    }];
    await installFixture(tempDir, fixture);

    const result = await getWorkRecordSummary({
      dir: tempDir,
      unit: "WK-9920#active-slice"
    });

    assert.equal(result.valid, true);
    assert.deepEqual(result.summary.selected_unit_summary.validation, [{
      operation: "node_test",
      target: "tests/active.test.mjs",
      verification_ids: ["claim-active"]
    }]);
    assert.equal(result.summary.selected_unit_summary.validation_count, 1);
    assert.equal(result.summary.selected_unit_summary.next_action, "continue implementation");
  });
});
