...
test("the registered content-reference route propagates the ranged-read unavailable owner", async () => {
  await withSpillEnv(async () => {
    const [clientTransport, serverTransport] = InMemoryTransport.createLinkedPair();
    const server = new McpServer({ name: "wiki-mcp-reference-refusal-test", version: "1.0.0" });
    const client = new Client({ name: "wiki-mcp-reference-refusal-client", version: "1.0.0" }, {
      capabilities: {}
    });

    registerMcpContentReferenceTools({
      registerTool: (name, config, handler) => server.registerTool(
        name,
        config,
        guardToolHandler(handler)
      ),
      z,
      jsonContent,
      errorContent
    ...
    try {
      ...
      const result = await client.callTool({
        name: "workspace_read_mcp_content_reference",
        arguments: { ref_id: "missing-content-reference", offset: 0, length: 1 }
      });

      assert.equal(result.isError, true);
      const { refusal, ...typedLimb } = result.structuredContent;
      ...
      assert.equal(facts["content_reference.failed_limb"], "ranged_read");
      assert.equal(refusal.no_supported_route, true);
      assert.equal(refusal.recovery.state, "no_supported_route");
      assert.equal(Object.prototype.hasOwnProperty.call(refusal, "next_calls"), false);
    } finally {
      await client.close();
      await server.close();
    }
...
test("installProcessErrorGuards logs process-level errors without throwing", () => {
  ...
  assert.ok(logged.every((entry) =>
    entry.message.includes("process-level invariant escaped its registered owner")));
  assert.ok(logged.every((entry) => entry.diagnostic_published === false),
    "with no injected sink, publication is explicitly reported as unavailable");
});
