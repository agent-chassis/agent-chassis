...
test("WK-1690 (review M-1): one shared lifecycle invocation records exactly one failure across concurrent status and wait", async () => {
  ...
  const [viaStatus, viaWait] = await Promise.all([
    fixture.status(),
    fixture.wait({ timeout_ms: 300, poll_interval_ms: 500 })
  ]);
...
test("WK-2201: run_wait's default window IS the owned monitor-call default, not a second copy of it", async () => {
  const { observed, tools } = createWindowObservingRegistry();

  const waited = parseStructuredTextResponse(await tools.get("workspace_agent_run_wait").handler({
    monitor_handle: NON_MANAGED_TERMINAL_RUN.monitor_handle,
  ...
  assert.deepEqual(observed, [MONITOR_CALL_DEFAULT_TIMEOUT_MS],
    "omitting timeout_ms must resolve the window from the one owned default");
...
test("WK-2201: an explicit timeout_ms still reaches the backend verbatim and is never replaced by the default", async () => {
  ...
  await tools.get("workspace_agent_run_wait").handler({
    monitor_handle: NON_MANAGED_TERMINAL_RUN.monitor_handle,
    subject: NON_MANAGED_TERMINAL_RUN.subject,
    timeout_ms: 1234
  });
...
test("WK-2201: the published timeout_ms range is the range the route enforces", async () => {
  ...
  const wait = async (timeoutMs) =>
    parseStructuredTextResponse(await tools.get("workspace_agent_run_wait").handler({
      monitor_handle: NON_MANAGED_TERMINAL_RUN.monitor_handle,
      subject: NON_MANAGED_TERMINAL_RUN.subject,
