...
test("Claude has no role-keyed exact-slice write-scope exception", () => {
  ...
  const exact = resolveClaudeLauncherWriteScope({
    role: "reviewer",
    writeScope: ["packages/implementation.mjs"],
    launcherOwnedFindings: true
  });
  assert.equal(exact.ok, false);
  assert.equal(exact.refusal.reason, "launcher_reviewer_write_scope_nonempty");
});
