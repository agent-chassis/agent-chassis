...
test("trusted integration fails closed when launcher-owned authorization is missing", async () => {
  ...
  await assert.rejects(defaultIntegrateManagedWorkerSlice({
    mainRepo: MAIN_REPO,
    ...
    deps: {
      ...
      runGit: async ({ args }) => args[0] === "merge-base"
        ? { ok: true, stdout: "" }
        : { ok: true, stdout: `${SHA}\n` },
    ...
    assert.match(error.message, /launcher-owned integration authorization is unavailable/u);
    return true;
  });

  assert.equal(integrationArguments, undefined);
  assert.equal(recordWrite, undefined);
});
