...
test('initiative frontier covers the implemented coordinator scenarios', () => {
  ...
  const expected = {
    ...
    'changes-requested advisory result': 'review_result_ready',
    'remediation ready for review': 'review_result_ready',
    'accepted-looking advisory result': 'review_result_ready',
    'record-level advisory result': 'review_result_ready',
  };
...
test('selected_action_id pulls the chosen action to the front', () => {
  ...
  assert.equal(reasonCodeOf(selected.top_actions[0]), 'review_result_ready');
});
...
async function freshInitiativeFixture({ withCorpus = false, status = 'todo' } = {}) {
  const root = await mkdtemp(path.join(os.tmpdir(), 'initiative-status-fresh-'));
  await mkdir(path.join(root, 'wiki', 'initiatives'), { recursive: true });
  await writeFile(
    path.join(root, 'wiki', 'initiatives', 'IN-9999.json'),
    `${JSON.stringify({
      id: 'IN-9999',
      ...
      updated: '2026-08-17',
      sections: {
        summary: 'Fresh initiative fixture.',
        goals: 'Exercise empty initiative status.',
        milestones: 'No work records yet.',
      },
    }, null, 2)}\n`,
  );
  if (withCorpus) {
...
const freshTaxonomy = loadInitiativeStatusTaxonomy({ repoRoot });

async function freshStatus(root) {
  return workspaceInitiativeStatus({
    repoRoot: root,
    initiative: 'IN-9999',
    taxonomy: freshTaxonomy,
  });
}

function assertZeroMemberCounts(result) {
  assert.deepEqual(result.counts, {
    actionable_unit_total: 0,
    actionable_unit_status_counts: {},
    record_corpus_total: 0,
    record_corpus_status_counts: {},
    slice_corpus_total: 0,
    slice_corpus_status_counts: {},
  });
  assert.deepEqual(result.consistency, []);
  assert.deepEqual(
    [result.consistency_total, result.consistency_returned, result.consistency_truncated],
    [0, 0, false],
  );
}

function assertAllocationRequired(result) {
  assertZeroMemberCounts(result);
  assert.deepEqual(result.top_actions, [{
    target_unit: result.scope.initiative,
    reason_code: 'allocation_required',
    suggested_tool: 'workspace_create_record',
  }]);
  assert.deepEqual(
    [result.top_actions_total, result.top_actions_returned, result.top_actions_truncated],
    [1, 1, false],
  );
  assert.deepEqual(result.next_action, result.top_actions[0]);
}

function assertNoAllocationRequired(result) {
  assertZeroMemberCounts(result);
  assert.deepEqual(result.top_actions, []);
  assert.deepEqual(
    [result.top_actions_total, result.top_actions_returned, result.top_actions_truncated],
  ...
  assert.equal(result.next_action.reason_code, 'no_pending_action');
  assert.notEqual(result.next_action.suggested_tool, 'workspace_create_record');
}

test('active initiative with absent work-record corpus requires allocation without creating it', async (t) => {
  const root = await freshInitiativeFixture();
  t.after(() => rm(root, { recursive: true, force: true }));
  const corpusPath = path.join(root, 'wiki', 'work-records');

  await assert.rejects(access(corpusPath), { code: 'ENOENT' });
  assertAllocationRequired(await freshStatus(root));
  await assert.rejects(access(corpusPath), { code: 'ENOENT' });
});

test('active initiative with an existing empty corpus requires the same allocation route', async (t) => {
  const root = await freshInitiativeFixture({ withCorpus: true, status: 'in_progress' });
  t.after(() => rm(root, { recursive: true, force: true }));
  assertAllocationRequired(await freshStatus(root));
});

test('closed or inactive initiatives with zero work records do not request allocation', async (t) => {
  for (const status of ['done', 'parked', 'cancelled', 'deprecated']) {
    const root = await freshInitiativeFixture({ status });
    t.after(() => rm(root, { recursive: true, force: true }));
    assertNoAllocationRequired(await freshStatus(root));
  }
});

test('unknown and noncanonical initiatives refuse instead of projecting valid empty status', async (t) => {
  const root = await freshInitiativeFixture();
  t.after(() => rm(root, { recursive: true, force: true }));
  ...
  await assert.rejects(
    ...
    (error) => error?.code === 'initiative_not_found' &&
      error?.envelope?.next_calls?.[0]?.tool === 'workspace_search_repo',
  );
  for (const initiative of ['IN-9999/../IN-9998', 'IN-1', 'DEC-9999']) {
    await assert.rejects(
      workspaceInitiativeStatus({ repoRoot: root, initiative, taxonomy: freshTaxonomy }),
      (error) => error?.code === 'initiative_identity_invalid',
      initiative,
    );
  }
});

test('loaded initiative identity mismatch remains a loud typed refusal', async (t) => {
  const root = await freshInitiativeFixture();
  t.after(() => rm(root, { recursive: true, force: true }));
  const initiativePath = path.join(root, 'wiki', 'initiatives', 'IN-9999.json');
  const mismatched = JSON.parse(await readFile(initiativePath, 'utf8'));
  mismatched.id = 'IN-9998';
  await writeFile(initiativePath, `${JSON.stringify(mismatched, null, 2)}\n`);

  await assert.rejects(
    freshStatus(root),
    (error) => error?.code === 'initiative_record_invalid',
  );
});

test('malformed and non-ENOENT corpus failures remain loud', async (t) => {
  const malformed = await freshInitiativeFixture({ withCorpus: true });
  const wrongType = await freshInitiativeFixture();
  t.after(() => Promise.all([
    rm(malformed, { recursive: true, force: true }),
    rm(wrongType, { recursive: true, force: true }),
  ]));
  await writeFile(path.join(malformed, 'wiki', 'work-records', 'WK-9998.json'), '{');
  await writeFile(path.join(wrongType, 'wiki', 'work-records'), 'not a directory\n');

  await assert.rejects(() => freshStatus(malformed), SyntaxError);
  await assert.rejects(() => freshStatus(wrongType), (error) => error?.code === 'ENOTDIR');
});

test('a later record-read ENOENT is not mistaken for an absent corpus', async (t) => {
  const root = await freshInitiativeFixture({ withCorpus: true });
  t.after(() => rm(root, { recursive: true, force: true }));
  const laterEnoent = Object.assign(new Error('record disappeared after enumeration'), {
    code: 'ENOENT',
  });
  ...
  assert.throws(
    () => loadInitiativeStatusRecords({
      repoRoot: root,
      readDirectory: () => [{ name: 'WK-9997.json', isFile: () => true }],
      readRecord: () => { throw laterEnoent; },
    }),
    (error) => error === laterEnoent,
  );
});

test('non-empty corpus filtering remains initiative-specific', async (t) => {
  const root = await freshInitiativeFixture({ withCorpus: true });
  t.after(() => rm(root, { recursive: true, force: true }));
  await Promise.all([
    writeFile(path.join(root, 'wiki', 'work-records', 'WK-9996.json'), JSON.stringify({
      id: 'WK-9996', initiative: 'IN-9999', status: 'todo',
    })),
    writeFile(path.join(root, 'wiki', 'work-records', 'WK-9995.json'), JSON.stringify({
      id: 'WK-9995', initiative: 'IN-9998', status: 'todo',
    })),
  ]);

  const result = await freshStatus(root);
  assert.equal(result.counts.record_corpus_total, 1);
  assert.equal(result.counts.actionable_unit_total, 1);
  assert.deepEqual(result.top_actions.map(({ target_unit }) => target_unit), ['WK-9996']);
  assert.equal(result.next_action.reason_code, 'record_needs_validation');
  assert.notEqual(result.next_action.suggested_tool, 'workspace_create_record');
});

test('a clean consumer repository with no work-record corpus returns allocation-required status', async (t) => {
  const root = await mkdtemp(path.join(os.tmpdir(), 'initiative-status-consumer-'));
  t.after(() => rm(root, { recursive: true, force: true }));
...
test('the package-owned action taxonomy resolves without any repository copy', async (t) => {
  const root = await mkdtemp(path.join(os.tmpdir(), 'initiative-status-taxonomy-'));
  t.after(() => rm(root, { recursive: true, force: true }));

  const taxonomy = loadInitiativeStatusTaxonomy({ repoRoot: root });
  assert.ok(Array.isArray(taxonomy.local_reason_codes) && taxonomy.local_reason_codes.length > 0);
  assert.ok(Array.isArray(taxonomy.action_kinds) && taxonomy.action_kinds.length > 0);
  assert.ok(Array.isArray(taxonomy.runtimeBlockerCodes) && taxonomy.runtimeBlockerCodes.length > 0);
  assert.equal(
    taxonomy.taxonomy_path.startsWith(root),
    false,
    'the taxonomy must not be resolved from the repository under inspection',
  );
});
