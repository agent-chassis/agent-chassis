...
test("WK-2347 readiness recovery vocabulary exposes no unavailable lifecycle continuation", () => {
  assert.equal(WORK_RECORD_DISPATCH_RECOVERY_STATE_VALUES.includes("operator_reconcile"), false);
  assert.equal(
    WORK_RECORD_DISPATCH_RECOVERY_STATE_VALUES.includes("establish_persistent_wk_lifecycle_ref"),
    false
  );
  assert.equal(WORK_RECORD_DISPATCH_RECOVERY_STATE_VALUES.includes("unavailable"), false);
});
