...
test("override token resolver does not turn app names into model selection", () => {
  const codex = resolveLauncherOverrideToken("codex");
  const claude = resolveLauncherOverrideToken("claude");
  assert.deepEqual(
    [codex, claude].map((result) => ({
      app: result.app ?? result.detail?.app ?? null,
      model: result.model ?? null,
      model_source: result.model_source ?? result.detail?.model_source ?? null
    })),
    [
      { app: "codex", model: null, model_source: null },
      { app: "claude", model: null, model_source: null }
    ]
  );
  ...
  assert.equal(agy.app, "agy");
  assert.equal(agy.model, null);
  assert.equal(agy.model_source, null);
