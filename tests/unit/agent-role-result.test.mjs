...
test("mints summary_budget only for reviewer and redteam results", () => {
  for (const reported_role of ["reviewer", "redteam"]) {
    const summary = "exact reviewer summary";
    const result = validateAgentRoleResult(basePayload({
      reported_role,
      summary,
      reviewed_controls: []
    ...
    assert.equal(result.valid, true);
    assert.deepEqual(result.result.summary_budget, {
      soft_limit_chars: 16_384,
      actual_chars: summary.length,
      exceeded: false
    });
  }

  const worker = validateAgentRoleResult(basePayload({
    reported_role: "worker",
    reported_outcome: "completed",
  ...
  assert.equal(Object.hasOwn(worker.result, "summary_budget"), false);
});

test("rejects child-supplied summary_budget without weakening ordinary invalid-result checks", () => {
  assertInvalid(
    validateAgentRoleResult(basePayload({ summary_budget: {
      soft_limit_chars: 16_384,
      actual_chars: 1,
      exceeded: false
    } })),
...
test("treats absent and null summaries as zero-length reviewer budgets", () => {
  const absent = basePayload({ reviewed_controls: [] });
  delete absent.summary;
  const absentResult = validateAgentRoleResult(absent);
  assert.equal(absentResult.valid, true);
  assert.deepEqual(absentResult.result.summary_budget, {
    soft_limit_chars: 16_384,
    actual_chars: 0,
    exceeded: false
  });

  const nullResult = validateAgentRoleResult(basePayload({ summary: null, reviewed_controls: [] }));
  assert.equal(nullResult.valid, true);
  assert.deepEqual(nullResult.result.summary_budget, {
    soft_limit_chars: 16_384,
    actual_chars: 0,
    exceeded: false
  });
...
test("uses JavaScript UTF-16 code-unit counting at the summary budget boundary", () => {
  for (const [summary, expected] of [
    ["a".repeat(16_384), { actual_chars: 16_384, exceeded: false }],
    ["a".repeat(16_385), { actual_chars: 16_385, exceeded: true }],
    ["😀".repeat(8_192), { actual_chars: 16_384, exceeded: false }],
    ["a".repeat(16_383) + "😀", { actual_chars: 16_385, exceeded: true }]
  ]) {
    const result = validateAgentRoleResult(basePayload({ summary, reviewed_controls: [] }));
    assert.equal(result.valid, true);
    assert.equal(result.result.summary_budget.actual_chars, expected.actual_chars);
    assert.equal(result.result.summary_budget.exceeded, expected.exceeded);
    assert.equal(result.result.summary, summary);
  }
});

test("preserves exact summary and findings while default capture accepts large valid results", () => {
  const summary = "x".repeat(140_000);
  const findings = [finding({
    title: "preserve this exact finding",
    severity: "low",
    blocking: false
  })];
  const payload = basePayload({
  ...
  const response = terminalFence(payload);
  const result = parseAgentRoleResult(response);

  assert.equal(result.valid, true);
  assert.equal(result.result.summary, summary);
  assert.deepEqual(result.result.findings, findings);
  assert.equal(result.result.summary_budget.actual_chars, summary.length);
  assert.equal(result.result.summary_budget.exceeded, true);

  const responseBytes = Buffer.byteLength(response, "utf8");
  const payloadBytes = Buffer.byteLength(JSON.stringify(payload), "utf8");
  assertInvalid(
    parseAgentRoleResult(response, { maxResponseBytes: responseBytes - 1 }),
    "response_oversized"
  );
  assertInvalid(
    parseAgentRoleResult(response, { maxPayloadBytes: payloadBytes - 1 }),
    "payload_oversized"
  );
});
