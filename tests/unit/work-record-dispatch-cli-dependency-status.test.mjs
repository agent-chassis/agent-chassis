...
test("validate-dispatch CLI strict mode consumes inline --dependency-status-json for record-address dispatch", async () => {
  await withTempRepo(async (tempDir) => {
    ...
    assert.equal(entry.target_work_kind, "implementation");
...
test("validate-dispatch CLI report mode routes --dependency-status-json through the same library path", async () => {
  await withTempRepo(async (tempDir) => {
    ...
    assert.equal(result.readiness.decision_code, "dispatchable");
...
test("validate-dispatch CLI keeps every canonical lifecycle status advisory", async () => {
  const statuses = ["inbox", "todo", "active", "review", "done", "blocked", "parked", "cancelled"];
  for (const status of statuses) {
    await withTempRepo(async (tempDir) => {
      const target = await installDependencyFixture(tempDir, "implementation-target.json", (mutable) => {
        mutable.status = status;
      });
      const consumer = await installDependencyFixture(tempDir, "implementation-with-record-dep.json", (mutable) => {
        mutable.depends_on = [target.id];
      });
      const output = await captureConsoleLog(() => runValidateDispatch([
        "strict", "--unit", consumer.id, "--dir", tempDir, "--json",
        "--graph-state-json", JSON.stringify(freshGraphState())
      ]));
      const result = JSON.parse(output);
      assert.equal(result.decision_code, "dispatchable", status);
      const entry = findDependencyEvidence(result).dependencies.find((item) => item.address === target.id);
      assert.equal(entry.provenance, "canonical_wk_json");
      assert.equal(entry.selected_status, status);
    });
  }
});

test("validate-dispatch CLI keeps the slice-DAG done frontier out of admission blocking", async () => {
  await withTempRepo(async (tempDir) => {
    const record = await installDependencyFixture(tempDir, "tracker-with-slice-deps.json", (mutable) => {
      mutable.slices[0].status = "active";
    });
    const projection = buildSliceDagProjection(record);
    assert.deepEqual(projection.frontier, ["prerequisite-slice"]);
    assert.deepEqual(projection.blocked, ["dependent-slice"]);
    assert.equal(projection.done.includes("dependent-slice"), false);

    const output = await captureConsoleLog(() => runValidateDispatch([
      "strict", "--unit", `${record.id}#dependent-slice`, "--dir", tempDir, "--json",
      "--graph-state-json", JSON.stringify(freshGraphState())
    ]));
    const result = JSON.parse(output);
    assert.equal(result.decision_code, "dispatchable");
...
test("validate-dispatch CLI keeps canonical outcome when supplied dependency facts conflict", async () => {
  await withTempRepo(async (tempDir) => {
    const target = await installDependencyFixture(tempDir, "implementation-target.json", (mutable) => {
    ...
    const output = await captureConsoleLog(() => runValidateDispatch([
      "strict", "--unit", consumer.id, "--dir", tempDir, "--json",
      "--graph-state-json", JSON.stringify(freshGraphState()), "--dependency-status-json",
      JSON.stringify({ [target.id]: { status: "blocked", work_kind: "implementation", provenance: "supplied" } })
    ]));
    const result = JSON.parse(output);
    assert.equal(result.decision_code, "dispatchable");
    const entry = findDependencyEvidence(result).dependencies.find((item) => item.address === target.id);
...
test("validate-dispatch CLI reports schema-invalid canonical targets as missing canonical facts", async () => {
  for (const workKind of [undefined, "not-a-canonical-work-kind"]) {
    await withTempRepo(async (tempDir) => {
      const target = await installDependencyFixture(tempDir, "implementation-target.json", (mutable) => {
        if (workKind === undefined) delete mutable.work_kind;
        else mutable.work_kind = workKind;
      });
      const consumer = await installDependencyFixture(tempDir, "implementation-with-record-dep.json", (mutable) => {
        mutable.depends_on = [target.id];
      });
      ...
      const entry = findDependencyEvidence(result).dependencies.find((item) => item.address === target.id);
      assert.equal(entry.marker, "fact_resolution_failed");
      assert.equal(entry.failure_code, "missing_canonical_target");
