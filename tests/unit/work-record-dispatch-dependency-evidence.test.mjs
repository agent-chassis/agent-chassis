...
test("dependency evidence covers the canonical work-record.v1 status enum without status-derived blocking", async () => {
  ...
  for (const sliceStatus of statuses) {
    await withTempRepo(async (tempDir) => {
      ...
      assert.equal(result.decision_code, "dispatchable", sliceStatus);
      assert.equal(result.dispatchable, true, sliceStatus);
    });
  }
});
...
test("intra-record slice dependencies missing from the canonical record fail closed", async () => {
  await withTempRepo(async (tempDir) => {
    ...
    assert.equal(entry.failure_code, "missing_canonical_target_slice");
...
test("inter-record dependencies missing from the local repo fail as canonical fact resolution", async () => {
  await withTempRepo(async (tempDir) => {
...
test("supplied dependency_statuses transport remains secondary to local canonical resolution", async () => {
  await withTempRepo(async (tempDir) => {
    const target = await installDependencyFixture(tempDir, "implementation-target.json");
    const consumer = await installDependencyFixture(
      tempDir,
    ...
    assert.equal(entry.provenance, "canonical_wk_json");
    assert.equal(entry.selected_status, target.status);
    assert.equal(entry.target_status, undefined);
    assert.equal(entry.supplied_status, undefined);
    assert.equal(entry.supplied_reason, undefined);
