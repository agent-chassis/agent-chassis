...
function makeLinkedMetadataFixture(prefix = "findings-git-metadata-") {
  ...
  mkdirSync(path.join(commonGitDir, "refs"), { recursive: true });
  mkdirSync(worktreeGitDir, { recursive: true });
  writeFileSync(path.join(commonGitDir, "HEAD"), "ref: refs/heads/main\n");
  writeFileSync(path.join(commonGitDir, "config"), "[core]\n\trepositoryformatversion = 0\n");
...
test("findings Git metadata resolves the linked checkout chain and recursive external alternates", () => {
  ...
  try {
    ...
    assert.deepEqual(
      ...
      [
        path.join(fixture.commonGitDir, "HEAD"),
        path.join(fixture.commonGitDir, "config"),
        path.join(fixture.commonGitDir, "refs"),
        fixture.primaryObjectDirectory,
...
test("findings Git metadata refuses a dead external gitdir and a fresh resolution succeeds", () => {
  const fixture = makeLinkedMetadataFixture();
  try {
