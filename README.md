...
- filesystem containment on macOS, which has no supported backend today
- diagnostics and refusal messages that name the violated contract and the next
  action
- drift-resistant tool discovery and configurable local readiness controls
- hosted Chassis Control Engine admission and signed attestation for the
  private-beta path
- durable orchestration for managed parallelism, retries, cancellation, and
  recovery

Per-package roadmaps are generated from canonical work records into each package
README.
