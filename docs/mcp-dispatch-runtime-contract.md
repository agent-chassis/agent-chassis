...
observed. A fresh retry settlement may preserve an exact-slice base from a
pre-spawn refusal while a canonical scope correction advances the moving WK
tip. That settlement is valid only when its planned base exactly matches the
authenticated slice binding and the retained base is an ancestor of the newly
authenticated WK tip. The allocated transition retains both identities, so the
current WK tip does not trigger duplicate allocation of the retained exact-slice
resources. An orphaned, divergent, unintegrated, or identity-mismatched slice
state remains a pre-spawn refusal. Stale plans and partial ref/worktree observations converge to the
