...
descriptor is inherited by number: readiness on fd 3, the authenticated no-CCE
declaration on fd 4 when present, and the common-proof resolver capability on fd
5. Their bytes cross unchanged with nothing on their path. The server's terminal
