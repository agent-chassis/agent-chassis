...
- `workspace_controlled_test_proof_authoring_describe({})` returns the package-
  derived native stable-v1 binding schema, vocabulary, and limits.
- `workspace_controlled_test_proof_query({wk_id, focus?, verification_ids})`
  selects only the named stable-v1 verification bindings and returns the carrier
  digest. It is also the complete retrieval path named by bounded runtime-
...
- `workspace_controlled_contract_authoring_state` keeps proof readiness nested
  in `authoring_evidence/non_authorizing_evidence`. It may report structurally
  complete authoring with proof execution `not_ready`; candidate projections
  include `candidate_total` and point to the exact verification-scoped query
  above. The lifecycle and four-state semantics are owned by
...
- `workspace_controlled_contract_runtime_prove` is the post-integration
  ...
  Before any provider runs, all exact integrated test-proof bindings must be
  package-ready. One nonready member mechanically refuses the indivisible
  complete attempt and produces no receipts. See
  [Test-proof runtime identity](test-proof-runtime-identity.md#stable-v1-readiness-lifecycle)
  for the semantic contract; this page owns only the route behavior.
...
accepts required canonical `subject`, optional `repo`, and orchestrator-only
optional `git_sha`. `subject` may identify a WK, slice, test proof, or obligation;
all four resolve through canonical relationship data to one deterministic proof
population and one aggregate result shape.
The request is closed: undeclared members and caller-selected target,
...
launcher-bound candidates. The server joins every selected obligation row,
controlled behavior and `verifies` relation, test-execution verification,
declared Node-test target, stable test-proof binding, selected worktree,
complete runtime receipts, admitted post-delivery pack, and content-addressed
evaluator. It deduplicates identical proofs while retaining every relationship.
...
`{ operation: "node_test", target, verification_ids }`; there is no command
string or second validation carrier. `target` must be one POSIX
...
whitespace, and control characters are rejected. Plain strings and
`{ note, verification_ids }` objects are human validation notes and never
authorize execution. Command-bearing objects, unknown operations, extra fields,
and `sections.structured_validation` are rejected.
...
all-zero identities, missing objects, and non-commit objects do not select
another source. The same purpose-neutral substrate used by exact-commit review
resolves the commit/tree from Git object data, creates a private detached
worktree under the launcher-owned root, authenticates its commit/tree, and
cleans it after execution. Existing worktrees at the commit are not selected or
inspected and their dirtiness is irrelevant. Without `git_sha`, the configured
repository worktree is selected directly whether clean or dirty and its identity
is checked before and after execution without mutation.

The complete population is preflighted through the work record package readiness
owner before any attempt context or process exists. One missing or invalid
inventory/selection, target, provider, evaluator, or relationship refuses the
whole call. The operation may spawn only joined declared targets through the shared
...
candidate. Worktree movement, identity mismatch, and confinement failure refuse
loudly. Exact-SHA calls create and remove one private worktree and write Git
worktree administration state through the shared lifecycle; omitted-SHA calls
perform none of those writes. The public result contains the requested subject,
one common subject-binding summary, aggregate status/counts, and compact
proof/verification identity and status rows. Non-satisfied rows add only a
stable reason code and minimal recovery facts. It never repeats common worktree
or generation facts per proof, exposes inventories, output, stacks, absolute
paths, commands, argv, environment, or raw runtime payloads, and never exceeds
65,536 UTF-8 bytes. Results remain non-authoritative `satisfied`,
`unsatisfied`, reason-coded `not_executable`, or typed refusal. Every distinct
proof executes once, only its predeclared stable identity supplies evidence,
and the complete observed node:test population remains in audit evidence. A
zero-proof subject refuses. Copied responses
...
Every modeled execution wrapper has a terminal bounded projection and never
degrades to `mcp_response.handler_exception.v1`. A valid
structured selected-test failure remains proof evidence and produces an
`unsatisfied` proof/relationship/aggregate result. A run that could not start,
timed out, lacked an authentic complete reporter summary, failed a provider
prerequisite, moved identity, or yielded an incomplete/cross-bound receipt is
`not_executable` or refused and never successful. The response retains only
minimal stable reason and recovery facts. Consult
`workspace_agent_faq({related_code:<returned-code>})` before retrying after the
owning prerequisite is repaired.

The proof provider runs the single declared file with Node's inner test
isolation disabled inside the existing launcher confinement. Only the
predeclared selected assertion determines proof pass/failure; sibling results
and the whole-file exit code remain bounded audit evidence.
`test_proof_selected_identity_not_observed` retains only the expected stable
test ID, target, file-wrapper status/error codes, observed/returned/omitted
counts, and at most eight deterministic safe relative file/name/nesting/ID
candidates when the file ends before that assertion appears.
`test_proof_bound_identity_mismatch` is reserved for a genuine
declared-versus-authenticated identity disagreement. Canonical discovery and
reporter observation share the same path normalization and identity hash; a
nesting-zero parent with nested `t.test(...)` children remains independently
observable, and no child is substituted for it.

The proof reporter uses a launcher-owned protocol pipe separate from test
diagnostics. Its envelope remains
lossless through parsing and digest/nonce authentication up to the fixed 2 MiB
proof-protocol ceiling; generic validation stdout and proof stderr continue to
use the 262,144-byte head/tail diagnostic bound. Protocol overflow drains the
child, retains no partial envelope, and projects
`test_proof_structured_events_oversized` with bounded truncation facts and
reporter-protocol recovery rather than malformed-JSON recovery.

The aggregate `contract_generation` is exclusively the
`controlled-contract-generation.v1` `generation_digest` returned by the
canonical generation owner. A carrier-set manifest `generation.id` or
`generation.path` only selects stored bytes and is not comparable to that
digest. Contract-carrier `content_digest` authentication remains independent.
Binding failures identify the exact failed status, count, identity, content, or
generation invariant with bounded expected/observed facts. Binding authoring
defects route to controlled test-proof query/patch; carrier contradictions route
to canonical repair; restart is suggested only for observed generation movement.
The public generation digest continues to cover only the canonical sorted
filename/content-digest population. For runtime minting, the canonical
carrier-set owner separately supplies opaque authenticated member-location and
storage-mode facts, so the launcher verifies manifest-selected bytes in the
execution candidate without constructing a root carrier path. Public test-proof
queries do not expose those runtime-only source facts. An absent canonical
contract returns `verify_proof.controlled_contract_carrier_absent.v1` with only
the canonical requested subject, carrier kind, and the recovery to author the
canonical controlled contract and proof population before retrying.
