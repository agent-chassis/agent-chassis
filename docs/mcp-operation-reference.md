...
- `workspace_work_record_set_status` writes a narrow trusted status update to a work-record or slice in a configured repo alias; it accepts `unit` plus `status`, reuses the canonical work-record edit substrate, and rejects arbitrary JSON patching or direct field-path edits. When the write moves the unit to `review` or `done`, the response carries an advisory post-write `closeout_lint` summary: `ok`/`valid`, `warning_count`, `error_count`, bounded `top_findings`, `generated_views`, and `next_action`, plus a top-level `cleanly_closeable` flag. The lint runs `generate_and_lint` first so generated views are refreshed before linting. This is advisory, not a hard refusal — the status write still succeeds — but if lint is red (`closeout_lint.ok:false`) the unit is **not** cleanly closeable until lint is fixed or a specific pre-existing lint blocker is recorded. Other status transitions report `closeout_lint.applicable:false`. When the record carries `completion_policy: forge_confirmed_merge`, a record-level `done` refuses with `forge_confirmed_completion_required` and no mutation; this route exposes that refusal but cannot perform forge closeout (see [Forge-confirmed completion](#forge-confirmed-completion)). Slice `done` and `done` on records without the policy are unchanged. Side effects: `workspace_write`, `record_write`. CLI fallback: `npm run wiki -- work-records set-status --unit <WK-ID|WK-ID#slice> --status <status>`
...
- `workspace_work_record_ready_slice` atomically creates or updates one complete, independently executable tracker-local slice contract. Its one strict schema exposes only the fields in `ready-slice-contract.v1`; it has no opaque slice payload, arbitrary patch, caller-selected write authority, or CLI fallback. It performs at most one canonical contract write and returns the closed `ready-slice-structural-readiness.v1` projection after a successful write or no-op. It is the only route that authors the optional record-level `completion_policy` (see [Forge-confirmed completion](#forge-confirmed-completion)). It is installed and supported in the free/local tier for orchestrator and operator sessions only. Side effects: `workspace_write`, `record_write`.
...
- `workspace_wk_forge_handoff` is the orchestrator/operator-only exact terminal-candidate publication route. Its request carries only `assigned_unit` plus an optional workspace alias; the request is not authorization. The launcher-owned host executor derives exact `C/B/W`, candidate materialization, candidate-bound record digest, repository, remote, base, branch, pull-request identity, and credentials. Reviewer/redteam results remain exact-candidate-bound advisory evidence: clean output cannot authorize, findings cannot veto, and missing or mixed results decide nothing. CCE alone decides any configured organization-policy gate; configured missing, unavailable, malformed, unratified, denied, or target-mismatched evidence fails closed. Paid tier alone configures no gate. No configured gate follows decision free-substrate behavior and reports a non-audit posture. Cold recovery reads only `refs/agent-launch/terminal-current-v2/<WK>` and accepts only an already-present, directly commit-valued raw target whose immutable C metadata mechanically authenticates the exact repository, B, W, tree, sole parent, and canonical contract. If that ref is absent, cold recovery fails closed with `terminal_candidate_recovery_current_ref_absent`; it never reconstructs currentness. Candidate construction from an absent ref occurs only during the hot post-worker lifecycle, where launcher-bound B/W/contracts exist and absence is the expected-old CAS state. Legacy candidate refs remain unread. Later landing-ref movement does not invalidate review or block publication of unchanged C; the configured merge actor and CCE policy own merge readiness. The exact candidate is published byte-for-byte, and exact branches and pull requests recover without duplication. Missing, ambiguous, moved, or inconsistent required Git/object/record/remote facts refuse. No Git, policy, reviewer, or forge authority is accepted from caller input, and credentials or raw process output never enter results. Denied to worker/reviewer/redteam.
...
Status and wait preserve the existing scalar `next_action` and may also return
the bounded `closeout_continuation` advisory. `ordered_steps` describes the
remaining process, while `current_safe_call` appears only for the step that is
safe now. The continuation grants no authority and performs no step itself.

For a parked worker the order is findings-only slice review, coordinator
disposition whenever the retained canonical outcome is `changes_requested`,
`workspace_integrate_committed_slice`, then the original worker monitor. Review
findings are advisory, coordinator disposition is separate, and integration is
a separate operation. An unresolved disposition sets `decision_required:true`
and leaves integration out of `current_safe_call`.

After terminal-slice integration, the finalized worker response promotes the
launcher-owned exact terminal-review dispatch already carried by lifecycle
state. Only a retained canonically eligible clean terminal result with a valid
structured `review_result` may make `workspace_wk_forge_handoff` the current
safe call. A canonical `changes_requested` result instead requires coordinator
disposition/remediation and leaves handoff out of `current_safe_call`; null,
invalid, malformed, incomplete, or otherwise ineligible retained results produce
no forge recommendation. Findings remain advisory in all cases: they neither
authorize nor veto a boundary operation.

A successful or idempotently recovered
`workspace_integrate_committed_slice` result carries the continuation inside its
existing `integration` result. It names `workspace_agent_run_status` and
`workspace_agent_run_wait` as alternatives for resuming the original worker
monitor and explicitly requires reuse of the launcher-minted original handle;
the integration route neither accepts nor returns that handle. Refused or failed
integration has no success continuation.

Candidate construction and forge handoff remain launcher-owned. None of these
continuations changes integration authorization, CCE policy, review semantics,
candidate identity, or forge behavior.
...
| `completion_policy` | `forge_confirmed_merge` or omitted | Optional; valid only while shaping the `terminal_whole_wk` review slice. | Same. Any other placement refuses `ready_slice_completion_policy_incompatible`; unknown values refuse. |
...
`completion_policy` is the optional record-level `decision` policy; its only
value today is `forge_confirmed_merge`, and the core planner remains the
authority on accepted values and placement. `workspace_work_record_ready_slice`
is the only route that authors it, and only while shaping the parent's
`terminal_whole_wk` review slice: any other placement refuses
`ready_slice_completion_policy_incompatible`, a slice object carrying the field
is structurally invalid, and `workspace_work_record_shape_review_unit` has no
such field in its closed input schema.

Once set, an ordinary policy-bound parent transition to `done` — including
through `workspace_work_record_set_status` — refuses with
`forge_confirmed_completion_required` and writes nothing. Slice status
transitions, parent `done` on a record without the policy, and other
transitions out of `review` are unchanged.

The parent reaches `done` before the merge, not after it: terminal review
applies to the exact candidate `C`, forge handoff publishes `C` unchanged, and
`work record` appends a review-state WK-only commit and then a status-only
completion WK commit to the handoff branch. Only after both does the helper
merge the exact pull-request head into the configured base branch, and the
`done` record becomes canonical only when that merge succeeds. No post-merge
completion commit is created; local reconciliation afterwards copies the exact
merged record state.

The policy is an operational routing guard, not evidence, a security token,
merge authority, or a cryptographic identity carrier. See `decision` and
