...
- `workspace_wk_forge_handoff` is the orchestrator/operator-only post-terminal WK forge handoff route (work record). Its request carries ONLY `assigned_unit` (a `WK-####` record selector); an optional `repo` alias selects the workspace but is never forge authority. It is a THIN CLIENT of the launcher-owned outside-bwrap host-write sidecar: the wiki-MCP process runs no Git or gh, reads no credential, and forwards `assigned_unit` across the existing `AGENT_LAUNCH_HOST_WRITE_AUTHORITY_TCP_ENDPOINT` to one closed `wk_forge_handoff` broker operation. The host broker — from its writable host namespace, using only the launcher-frozen `mainRepo`, the canonical WK record, Git truth, and gh — resolves the canonical HTTPS `origin` coordinate, proves terminal/quiescent eligibility (no implementation slice can still advance the persistent WK ref, every integrated contribution is represented by the WK tip, and a canonical initiative is assigned), reuses the frozen WK tip tree with the unique merge base as the sole parent to build one deterministic squash commit (fixed identity/message/dates, no wall clock), publishes one forge branch by absent-ref CAS, and creates or recovers exactly one pull request. The forge and organization own review, approvals, checks, branch protection, merge queues, and merge. The result is one of four typed outcomes — `no_changes`, `handed_off`, `human_action_required` (a bounded manual `git merge --ff-only` argv when host gh is absent/unauthenticated AND a fast-forward is mechanically proven), or `human_reconciliation_required` (no guessed branch and no merge command otherwise). When the launcher-owned sidecar endpoint is absent the route returns a typed `backend_unavailable` refusal; the gh token is never read, returned, placed on an argv, or put on the wire. There is no shell/wrapper fallback. Side effects: `process_spawn` (host-side only). Denied to worker/reviewer/redteam.
...
record to retain the pre-integration active/review shape. Enforced-CCE recovery also
independently resolves persisted historical Proof A against the frozen contract,
runs, identity, and SHAs; `policy_only` fabricates no proof or audit acceptance, and
no public request carries proof. Claude exact-slice review exposes its credential
