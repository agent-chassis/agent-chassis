...
- **Simpler first use.** Bootstrap no longer generates or preserves a repo-local `docs/adoption.md`. It seeds only the in-progress `IN-0001` first-work placeholder and creates no work record, slice, projection, or work-record allocator reservation. The first normal allocator-backed work record therefore receives `WK-0001`; the retired adoption tracker and `wiki adoption verify` lifecycle are not shipped.
